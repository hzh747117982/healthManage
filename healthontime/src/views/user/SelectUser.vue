<template>
  <div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="number" label="账号" width="180" :formatter="formatnumber">
      </el-table-column>
      <el-table-column prop="name" label="姓名" width="180" :formatter="formatPatientName">
      </el-table-column>
      <el-table-column prop="age" label="年龄" width="80" :formatter="formatAge">
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    let checkDuplicate = (rule, value, callback) => {
      ...
      this.$axios.get('http://localhost:8188/user/findByNo?no='+this.form.no).then(res=>res.data).then(res => {
        if (res.code != 200) {
          callback();
        } else {
  ...
  methods: {
    ...
    del(id){
      this.$axios.get('http://localhost:1/user/delete?id='+id).then(res => res.data).then(res => {
        console.log(res.data)
        if (res.code === 200) {
          this.tableData = res.data
          this.total = res.total
