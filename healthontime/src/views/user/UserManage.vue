    ...
    <el-table :data="tableData"
      ...
      <el-table-column prop="operate" label="操作" width="180">
        <template slot-scope="scope">
          ...
          <el-popconfirm title="这是一段内容确定删除吗？" @confirm="del(scope.row.userId)" style="margin-left: 5px">
...
export default {
  ...
  methods: {
    ...
    loadGet() {
      this.$axios.post('http://localhost:8188/user/listPage', {
    ...
    del(id){
      this.$axios.get('http://localhost:8188/user/delete?userId='+id).then(res => res.data).then(res => {
        console.log(res.data)
        if (res.code === 200) {
          this.tableData = res.data
          this.total = res.total
