<template>
  <div>
    <div style="margin-bottom: 5px;">
      <el-input v-model="name" placeholder="请输入药品分类名" suffix-icon="el-icon-search" style="width: 200px;"
...
export default {
  ...
  methods: {
    ...
    loadGet() {
      this.$axios.post('http://localhost:8188/medicineType/listPage', {
    ...
    save() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$axios.post('http://localhost:8188/medicineType/saveOrMod', this.form).then(res => res.data).then(res => {
            console.log(res.data)
            if (res.code === 200) {
              this.tableData = res.data
              this.total = res.total
